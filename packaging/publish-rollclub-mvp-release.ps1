...
- виправлено чергування (Ctrl+F4): усунуто колізію Панелі стовпців («Панель столбцов» / «Column Panel») з Панеллю даних — через збіг слова «панель» зчитувач замість замовлень брав заголовки стовпців, через що показувало «всього 0» і щосекунди скидався кеш з 11-секундним пошуком
- кешування таблиці gridDeliveries усуває 11-секундне зависання між опитуваннями і робить дежурство швидким і стабільним
- відновлено пріоритет відкаліброваних оператором координат PoiskX/PoiskY та RowX/RowY: номер вводиться точно в рядок автофільтра, а подвійний клік відкриває саме перший відфільтрований рядок без залізання в чужі замовлення
- додано цикл очікування звуження списку (до 2.5 с) після введення номера, щоб Syrve встигав відфільтрувати замовлення і воно не пропускалося
- знято обмеження замовлень тільки зі словом "пост": дежурство успішно бере замовлення по Дніпру, Києву та іншим містам
- покращено логування дій чергування в ahk_debug.log та bridge.log для миттєвої діагностики на робочих місцях операторів
