...
- виправлено чергування (Ctrl+F4): відновлено швидке зчитування замовлень без циклічного перемикання вкладок, точне введення номера в рядок пошуку та подвійний клік по першому рядку колонки № для надійного відкриття замовлення
- додано підтримку україномовної локалізації Syrve (Панель даних, Рядок, Коментар): дежурство працює швидко та коректно розпізнає замовлення з коментарем «пост»
