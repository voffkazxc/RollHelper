...
- виправлено чергування (Ctrl+F4): усунуто проблему "всього 0 замовлень" на україномовних та модифікованих інтерфейсах Syrve (розпізнавання рядків незалежно від регістру "рядок"/"строка", колонок "№"/"Номер")
- знято обмеження замовлень тільки зі словом "пост": тепер дежурство успішно бере замовлення по Дніпру, Києву та іншим містам
- покращено логування дій чергування в ahk_debug.log та bridge.log для миттєвої діагностики на робочих місцях операторів
- точне введення номера в рядок автофільтра та подвійний клік по першому рядку звуженого списку для надійного взяття
