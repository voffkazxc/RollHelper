param(
    [string]$CatalogVersion = "0.1.83",
    [string]$RollClubVersion = "0.1.42",
...
- повністю ліквідовано циклічне перемикання вкладок (Ctrl+Tab) у режимі чергування: виключено випадковий перехід у вкладки Bolt, звітів або інших вікон під час роботи
- запроваджено тихе фонове опитування: за відсутності нових замовлень або при відкритій картці скрипт не активує вікно, не тисне Esc і не рухає мишу, забезпечуючи спокійну роботу оператора без спаму
- додано автоматичне ігнорування агрегаторів: замовлення Bolt, Glovo та Ракета пропускаються та не беруться в чергування
- посилено розпізнавання відкритої картки замовлення через глибинний пошук DeliveryOrderEditControl (глибина 15), що усуває відкриття кількох замовлень одночасно
- додано перевірку скритності таблиці: якщо список доставок прихований або неактивний, фонові фантомні замовлення не читаються
- додано безпечне очищення фільтра при збої звуження або відкриття із затримкою 2 секунди замість нескінченного спаму
- додано жорсткий бар'єр безпеки: перед натисканням тільди (TriggerMain) та внесенням паличок скрипт перевіряє факт реального відкриття картки замовлення через сервер. Якщо картка не відкрилася, внесення блокується, хаотичні кліки по вікну колл-центру виключено
- виправлено помилкове виділення всіх рядків: команду Ctrl+A замінено на комбінацію Home+End+Delete та BackSpace, яка безпечно очищує рядок пошуку без виділення сітки в Syrve
- виправлено ігнорування позначки «перезвонить» на російськомовному Syrve: додано розпізнавання колонки «Примечание» («Примітка») та наскрізний пошук маркерів зворотного дзвінка (перезвон, передзвон, позвонить, подзвон) по всіх клітинках рядка
- додано фільтр статусів: беруться тільки нові непідтверджені замовлення (Не подтверждена, Не підтверджена, Новая, Нова, Не обработана); замовлення «Готовится», «В пути», «Доставлен», «Закрыт» більше не беруться
- додано автоматичний резервний клік відкриття: якщо калібровані координати RowX/RowY не відкрили замовлення, скрипт пробує реальні екранні координати рядка від UIA
- виправлено колізію Панелі стовпців з Панеллю даних та кешовано gridDeliveries для швидкого опитування без 11-секундних затримок
- знято обмеження замовлень тільки зі словом "пост": дежурство бере вільні замовлення по всіх містах
- налаштування, координати та WinAPI-прив'язки зберігаються в UserData і не губляться після оновлення
