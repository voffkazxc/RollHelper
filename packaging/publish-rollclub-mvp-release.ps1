param(
    [string]$CatalogVersion = "0.1.82",
    [string]$RollClubVersion = "0.1.41",
...
- додано жорсткий бар'єр безпеки: перед натисканням тільди (TriggerMain) та внесенням паличок скрипт перевіряє факт реального відкриття картки замовлення через сервер. Якщо картка не відкрилася (наприклад, через невідкалібровані координати на новому ноутбуці), внесення блокується, хаотичні кліки по вікну колл-центру виключено
