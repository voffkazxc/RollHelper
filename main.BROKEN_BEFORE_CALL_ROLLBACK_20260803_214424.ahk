...
if (aikoInputX = 0 || aikoInputX = "" || aikoInputX = "ERROR")
    aikoInputX := phoneX, aikoInputY := phoneY
if (aikoCallX = 0 || aikoCallX = "" || aikoCallX = "ERROR")
    aikoCallX := callX, aikoCallY := callY
if (aikoHangupX = 0 || aikoHangupX = "" || aikoHangupX = "ERROR")
    aikoHangupX := callEndX, aikoHangupY := callEndY
...
IniRead, callListenDevice, RkConfig.ini, CallListen, DeviceIndex, 
...
DrawRollclub:
    ...
    Gui, Roll:Add, Button, x262 y19 w28 h28 gCallToggleMode, ☎
...
OpenSettings:
    ...
    Gui, Settings:Add, Text, xs y+6 w150 h22 +0x200, Пристрій слуху:
    Gui, Settings:Add, Edit, x+6 yp w150 vNewCallListenDevice, %callListenDevice%
    Gui, Settings:Add, Text, xs y+2 w316 c%RhC_Muted%, Подивитись номери: tools\call_listener\list_audio_devices.bat
...
SaveSettings:
    ...
    IniWrite, %NewCallListenDevice%, RkConfig.ini, CallListen, DeviceIndex
    if (NewCashSearch != "")
...
; ════════════════════════════════════════════════════════════
RhEnsureAikoTargets() {
    global BRAND_DIR, aikoInputX, aikoInputY, aikoCallX, aikoCallY, phoneX, phoneY, callX, callY, logFile
    _cfg := BRAND_DIR . "\RkConfig.ini"
    if (aikoInputX = 0 || aikoInputX = "" || aikoInputX = "ERROR") {
        IniRead, _x, %_cfg%, Targets, AikoInputX, %phoneX%
        IniRead, _y, %_cfg%, Targets, AikoInputY, %phoneY%
        aikoInputX := _x
        aikoInputY := _y
    }
    if (aikoCallX = 0 || aikoCallX = "" || aikoCallX = "ERROR") {
        IniRead, _x, %_cfg%, Targets, AikoCallX, %callX%
        IniRead, _y, %_cfg%, Targets, AikoCallY, %callY%
        aikoCallX := _x
        aikoCallY := _y
    }
    if (aikoInputX = 0 || aikoCallX = 0 || aikoInputX = "ERROR" || aikoCallX = "ERROR") {
        FileAppend, `n[WARN] %A_Now% - Call mode skipped: missing Aiko targets. input=%aikoInputX%x%aikoInputY% call=%aikoCallX%x%aikoCallY%`n, %logFile%
        return 0
    }
    return 1
}
...
*F6::
*F8::
    GoSub, CallToggleMode
return

CallToggleMode:
    FileAppend, % "[" . A_Now . "] CALL_HOTKEY pressed. callListMode=" . callListMode . "`n", %A_ScriptDir%\ahk_debug.log
    if (callListMode) {
        ; --- ВИМКНУТИ режим ---
        GoSub, CallStopMode
        return
...
ShowCallProgress:
    ...
    Gui, CallGui:Add, Button, x10 y+6 w260 h28 gCallListenTest, 🧪 Тест слуху 8 сек
...
CallListenTest:
    ToolTip, 🎧 Слухаю 8 секунд... не говори поверх клієнта, 10, 10, 6
    _listenCmd := ComSpec . " /c cd /d """ . A_ScriptDir . """ && python ""tools\call_listener\call_listen.py"" --seconds 8"
    ...
    if FileExist(_listenTxtFile) {
        ...
        MsgBox, 64, Тест слуху, %_listenText%
    } else {
        MsgBox, 48, Тест слуху, Результат не створено.`nПеревір python / tools\call_listener.
    }
    ToolTip, , , , 6
return
