...
CallListenTest:
    ...
    _listenCmd := ComSpec . " /c cd /d """ . A_ScriptDir . """ && python ""tools\call_listener\call_listen.py""" . _deviceArg . " --model small --language auto --beam-size 3 --until-silence --seconds 8 --start-timeout 4 --min-speech-seconds 1.5 --silence-seconds 1.0 --rms-threshold 120"
    RunWait, %_listenCmd%, %A_ScriptDir%, Hide
    _listenTxtFile := A_ScriptDir . "\data\call_listener\last_result.txt"
    if FileExist(_listenTxtFile) {
        FileRead, _listenText, %_listenTxtFile%
        MsgBox, 64, Слух клієнта, %_listenText%
    } else {
        MsgBox, 48, Слух клієнта, Результат не створено.`nПеревір tools\call_listener\call_listen.py
    }
    GuiControl, CallGui:, CallStatusLbl, ⏳ Очікування...
return
...
CallDetectChannel:
    ...
    _detectCmd := ComSpec . " /c cd /d """ . A_ScriptDir . """ && python ""tools\call_listener\call_listen.py"" --model small --language auto --beam-size 1 --until-silence --seconds 5 --start-timeout 0.7 --min-speech-seconds 0.2 --silence-seconds 0.4 --rms-threshold 120 --no-transcribe"
    ...
    if (_devIndex != "" && _devIndex != "ERROR" && _peakRms > 0 && _vadReason != "start_timeout") {
        callListenDevice := _devIndex
        IniWrite, %callListenDevice%, RkConfig.ini, CallListen, Device
        GuiControl, CallGui:, CallStatusLbl, % "✅ Канал [" . _devIndex . "] збережено"
        GuiControl, CallGui:, CallCrmLbl, % SubStr(_devName, 1, 42)
    } else {
...
CallAutoListenAfterGreeting:
    ...
    if (_decision = "positive") {
        ...
        _byeCmd := ComSpec . " /c cd /d """ . A_ScriptDir . """ && python ""tools\call_listener\call_listen.py""" . _deviceArg . " --until-silence --seconds 3 --start-timeout 1 --min-speech-seconds 0.4 --silence-seconds 1.0 --rms-threshold 120 --no-transcribe"
