...
; (ImageSearch-блок для SivClickStick/Soy/Ginger/Wasabi видалено — більше не використовується)
...
RcVerifyPoint:
    ...
    if (isOk && sessionId != "") {
        ...
        ; Якщо COLLECT успішний і точка прочитана (навіть порожня, але валідна), вважаємо успіхом
        if (cOk) {
            iikoPoint := pointStr
            diagSuccess := true
        } else {
            FileAppend, % "[" A_Now "] RcVerifyPoint COLLECT FAIL: " _diagRes "`n", %A_ScriptDir%\siv_debug.log
        }
    }
