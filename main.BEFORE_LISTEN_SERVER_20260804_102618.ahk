...
RhUiaClickPaymentTypeRow() {
    root := RhUiaFind("gridPaymentItems")
    if (IsObject(root)) {
...
global noPayChange := 0   ; 1 = списання бонусів: НЕ чіпаємо тип оплати (не тиснемо хрестик, не міняємо тип)
...
RhBuildSivPreview() {
    ...
    if ((itemX = 0 || itemX = "ERROR") && !IsObject(RhUiaFind("treeListItems")))
        _txt .= " · !приціл 7"
    return _txt
}
...
RhEnterPreflight:
    ...
    if (RhOneLine(_pfComment) != "" && !RhTargetReady(commX, commRelX) && !IsObject(RhUiaFind("memoEditDeliveryComment")))
        _pf .= "- Коментар: приціл 1 не налаштований`n"
    if ((RhOneLine(_pfKitchen) != "" || RhOneLine(_pfCard) != "") && !RhTargetReady(infoX, infoRelX) && !IsObject(RhUiaFind("memoEditCustomerComment")))
        _pf .= "- Кухня: приціл 3 не налаштований`n"
    if (RhOneLine(_pfAddress) != "" && !RhTargetReady(addrX, addrRelX) && !IsObject(RhUiaFind("memoEditDeliveryAddressComment")))
        _pf .= "- Адреса: приціл 4 не налаштований`n"
    if (_pfHasTime && !RhTargetReady(timeX, timeRelX) && !IsObject(RhUiaFind("timeEditDeliveryTime")))
        _pf .= "- Час готовності: приціл 6 не налаштований`n"
    if ((_pfHasSiv || _pfHasGift) && !RhTargetReady(itemX, itemRelX) && !IsObject(RhUiaFind("treeListItems")))
        _pf .= "- Таблиця страв: приціл 7 не налаштований`n"
    if (_pfHasPay && !RhTargetReady(cashX, cashRelX) && !IsObject(RhUiaFind("gridPaymentItems")))
        _pf .= "- Тип оплати: приціл 9 не налаштований`n"
    if (_pfHasPay && !RhTargetReady(crossX, crossRelX) && !IsObject(RhUiaFind("buttonDeletePaymentItem")))
        _pf .= "- Хрестик оплати: приціл 8 не налаштований`n"
    if (DoAutoCash && isPickup && calcChange > 0 && !RhTargetReady(changeX, changeRelX) && !IsObject(RhUiaFind("gridPaymentItems")))
...
RhSaveEnterState:
    ...
    if (DoAutoCash)
        ...
        _stPay := "card"
    _stPayWillRun := ((DoAutoCash || DoAutoCard || paymentMethod != "") && !noPayChange) ? 1 : 0
...
OpenSettings:
    ...
    Gui, Settings:Add, Button, xs y+4 w158 h25 gSetNoChangeTarget, 11. Без сдачи
    Gui, Settings:Add, Button, x+6 yp w158 h25 gSetStreetTarget, 12. Вулиця (зона KML)
    Gui, Settings:Add, Button, xs y+4 w158 h25 gSetPhoneTarget, 13. Телефон клієнта
...
TestTargets:
    ...
    ShowTarget(noChangeX, noChangeY, "11. Без сдачи")
    ShowTarget(streetX, streetY, "12. Вулиця")
    ShowTarget(phoneX, phoneY,   "13. Телефон")
    ToolTip
...
SetNoChangeTarget:
    Gui, Settings:Hide
    Sleep, 300
    MsgBox, 4160, Налаштування, Клікни на кнопку 'Без сдачи' під списком оплат.
    KeyWait, LButton, Down
    MouseGetPos, noChangeX, noChangeY, _whWin
    IniWrite, %noChangeX%, RkConfig.ini, Targets, NoChangeX
    IniWrite, %noChangeY%, RkConfig.ini, Targets, NoChangeY
    WinGetPos, _wx, _wy,,, ahk_id %_whWin%
    noChangeRelX := noChangeX - _wx,  noChangeRelY := noChangeY - _wy
    IniWrite, %noChangeRelX%, RkConfig.ini, TargetsRel, NoChangeX
    IniWrite, %noChangeRelY%, RkConfig.ini, TargetsRel, NoChangeY
    Gui, Settings:Show
return
...
RhFinishOrder:
    ...
    IikoRestore()
    _finishOk := 1
    if (!RhUiaClick("buttonDeliveryConfirmation")) {
        _finishOk := 0
        ToolTip, % "⚠️ Не знайшов/не натиснув Подтвердить"
        FileAppend, % "[" . A_Now . "] RH_FINISH confirm failed`n", %A_ScriptDir%\ahk_debug.log
        Sleep, % SpDly(700)
    } else {
        Sleep, % SpDly(350)
    }

    if (!RhUiaClick("buttonSaveAndClose")) {
        _finishOk := 0
        ToolTip, % "⚠️ Не знайшов/не натиснув Сохранить на точку"
        FileAppend, % "[" . A_Now . "] RH_FINISH save failed`n", %A_ScriptDir%\ahk_debug.log
        Sleep, % SpDly(900)
    } else {
        ToolTip, % "✅ Підтверджено + збережено на точку"
        FileAppend, % "[" . A_Now . "] RH_FINISH ok`n", %A_ScriptDir%\ahk_debug.log
        Sleep, % SpDly(500)
    }
...
RollEnter:
    ...
    if (_hasSivInput && (itemX = 0 || itemX = "ERROR") && !IsObject(RhUiaFind("treeListItems"))) {
        MsgBox, 48, Помилка, Не задано приціл "Табл. Страв"! (Налаштування -> 7)
        RhEnterBusy := 0
        return
    }
...
SilentMagicClean:
    ...
    ; Списання бонусів → НЕ чіпати тип оплати при пробитті (сума не міняється, є 2 типи оплати)
    ; Будь-яка згадка бонусів (списання/оплата бонусами) → НЕ чіпати тип оплати.
    ; Знижка/промокод сюди НЕ входять (там другий тип оплати не створюється, оплату ставимо).
    noPayChange := RegExMatch(text, "i)(бонус|бонусами|бали)") ? 1 : 0
...
RhPunchPluSeries(jobs, itX, itY) {
    if (!IsObject(jobs) || jobs.MaxIndex() = "")
        return 1
    if ((itX = 0 || itX = "ERROR") && !IsObject(RhUiaFind("treeListItems")))
        return 0
...
ApplyRollclub:
    ...
    ; Робимо для ВСІХ замовлень (доставка і самовивіз), якщо відомий тип оплати або вибрано вручну.
    ; При списанні бонусів (noPayChange) НЕ чіпаємо тип оплати взагалі — лишаємо як є в iiko.
    if ((DoAutoCash || DoAutoCard || paymentMethod != "") && !noPayChange) {
        _paySearch := ""
        if (DoAutoCash)
        ...
        _payNoChange := (_payMode != "" && _isGiftPunched && !(_payMode = "cash" && isPickup && calcChange > 0)) ? 1 : 0
        if (!RhApplyPaymentUIA(_payMode, (_payMode = "cash" && isPickup) ? calcChange : "", _payNoChange)) {
            Sleep, % SpDly(180)
            ...
            if (isPickup && (DoAutoCash || (!DoAutoCard && InStr(paymentMethod, "Готівка"))) && calcChange > 0 && (changeX > 0 || changeRelX > 0)) {
                ...
                Send, {NumpadEnter}
                Sleep, % SpDly(180)
            } else if (_isGiftPunched && (noChangeX > 0 || noChangeRelX > 0)) {
                ; Всі інші (доставка, картка, оплачено, або готівка без сдачи) -> Без сдачи
                ; РОБИМО ЦЕ ТІЛЬКИ ЯКЩО БУВ ПОДАРУНОК (бо iiko сама підтягує правильну суму, якщо подарунка не було)
...
CallListenTest:
    ...
    GuiControl, CallGui:, CallStatusLbl, 🎧 Тест слуху...
    _deviceArg := ""
    if (callListenDevice > 0)
        _deviceArg := " --device " . callListenDevice
    _listenCmd := ComSpec . " /c cd /d """ . A_ScriptDir . """ && python ""tools\call_listener\call_listen.py""" . _deviceArg . " --model small --language ru --beam-size 3 --until-silence --seconds 6 --start-timeout 4 --min-speech-seconds 0.6 --silence-seconds 0.45 --rms-threshold " . callListenRmsThreshold
    ...
    if FileExist(_listenIni) {
        ...
        GuiControl, CallGui:, CallStatusLbl, % "🎧 Тест: " . _decision . " / " . _reason
        GuiControl, CallGui:, CallCrmLbl, % "[" . _devIndex . "] peak " . _peakRms . " | " . SubStr(_text, 1, 32)
    } else {
        GuiControl, CallGui:, CallStatusLbl, ❌ Тест не створив результат
        GuiControl, CallGui:, CallCrmLbl, Перевір tools\call_listener\call_listen.py
    }
...
CallDetectChannel:
    GoSub, CallDiagnosticGuardOn
    GuiControl, CallGui:, CallStatusLbl, 🔊 Перевіряю канал...
    GuiControl, CallGui:, CallCrmLbl, Увімкни звук у потрібному каналі
    _detectCmd := ComSpec . " /c cd /d """ . A_ScriptDir . """ && python ""tools\call_listener\call_listen.py"" --model small --language auto --beam-size 1 --until-silence --seconds 5 --start-timeout 0.7 --min-speech-seconds 0.2 --silence-seconds 0.4 --rms-threshold 3 --no-transcribe"
    ...
    IniRead, _peakRms, %_listenIni%, CallListen, VadPeakRms, 0
    if (_devIndex != "" && _devIndex != "ERROR" && _peakRms >= 5) {
        callListenDevice := _devIndex
        callListenRmsThreshold := Round(_peakRms * 0.45)
        if (callListenRmsThreshold < 5)
            callListenRmsThreshold := 5
        ...
        GuiControl, CallGui:, CallStatusLbl, % "✅ [" . _devIndex . "] peak " . _peakRms . " → поріг " . callListenRmsThreshold
        GuiControl, CallGui:, CallCrmLbl, % SubStr(_devName, 1, 38) . " | " . _vadReason
    } else {
        GuiControl, CallGui:, CallStatusLbl, % "❌ peak " . _peakRms . " < 5, не зберіг"
        GuiControl, CallGui:, CallCrmLbl, % "[" . _devIndex . "] " . SubStr(_devName, 1, 34) . " | " . _vadReason
    }
    GoSub, CallDiagnosticGuardOff
return
...
CallAutoListenAfterGreeting:
    ...
    GuiControl, CallGui:, CallStatusLbl, 🎧 Слухаю клієнта...
    _deviceArg := ""
    if (callListenDevice > 0)
        _deviceArg := " --device " . callListenDevice
    _listenCmd := ComSpec . " /c cd /d """ . A_ScriptDir . """ && python ""tools\call_listener\call_listen.py""" . _deviceArg . " --model small --language ru --beam-size 3 --until-silence --seconds 6 --start-timeout 4 --min-speech-seconds 0.6 --silence-seconds 0.45 --rms-threshold " . callListenRmsThreshold
    ...
    if (_decision = "positive") {
        if (callManualGoodbyeTick > 0) {
            GuiControl, CallGui:, CallStatusLbl, ✅ Positive, але X вже вручну
            GuiControl, CallGui:, CallCrmLbl, % SubStr(_text, 1, 48)
            callAutoListenBusy := 0
            SetTimer, RemoveCallProgressTip, -2500
            return
        }
        GuiControl, CallGui:, CallStatusLbl, ✅ Positive → прощання X
        GuiControl, CallGui:, CallCrmLbl, % SubStr(_text, 1, 48)
        Send, {x}
        Sleep, 3500
        GuiControl, CallGui:, CallStatusLbl, 👂 Чекаю тишу після X...
        _byeCmd := ComSpec . " /c cd /d """ . A_ScriptDir . """ && python ""tools\call_listener\call_listen.py""" . _deviceArg . " --until-silence --seconds 3 --start-timeout 1 --min-speech-seconds 0.4 --silence-seconds 1.0 --rms-threshold " . callListenRmsThreshold . " --no-transcribe"
        ...
        if (_byeVad = "silence_after_speech")
            _canHangupTest := 1
        if (_canHangupTest) {
            GuiControl, CallGui:, CallStatusLbl, ✅ Можна класти трубку (тест)
        } else {
            GuiControl, CallGui:, CallStatusLbl, ⚠️ Після X є звук — ручний режим
        }
    } else {
        GuiControl, CallGui:, CallStatusLbl, % "👤 Ручний режим: " . _decision . " / " . _reason
        GuiControl, CallGui:, CallCrmLbl, % "[" . _devIndex . "] " . SubStr(_devName, 1, 28) . " | " . SubStr(_text, 1, 28)
    }
    callAutoListenBusy := 0
