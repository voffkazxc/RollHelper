...
IniRead, callListenRmsThreshold, RkConfig.ini, CallListen, RmsThreshold, 18
...
CallListenTest:
    ...
    _listenCmd := ComSpec . " /c cd /d """ . A_ScriptDir . """ && python ""tools\call_listener\call_listen.py""" . _deviceArg . " --model small --language auto --beam-size 3 --until-silence --seconds 8 --start-timeout 4 --min-speech-seconds 1.5 --silence-seconds 1.0 --rms-threshold " . callListenRmsThreshold
...
CallDetectChannel:
    ...
    _detectCmd := ComSpec . " /c cd /d """ . A_ScriptDir . """ && python ""tools\call_listener\call_listen.py"" --model small --language auto --beam-size 1 --until-silence --seconds 5 --start-timeout 0.7 --min-speech-seconds 0.2 --silence-seconds 0.4 --rms-threshold 10 --no-transcribe"
    ...
    IniRead, _peakRms, %_listenIni%, CallListen, VadPeakRms, 0
    if (_devIndex != "" && _devIndex != "ERROR" && _peakRms >= 10) {
        callListenDevice := _devIndex
        callListenRmsThreshold := Round(_peakRms * 0.45)
        if (callListenRmsThreshold < 10)
            callListenRmsThreshold := 10
        ...
        GuiControl, CallGui:, CallStatusLbl, % "✅ Канал [" . _devIndex . "] RMS " . callListenRmsThreshold
        GuiControl, CallGui:, CallCrmLbl, % SubStr(_devName, 1, 38)
    } else {
        GuiControl, CallGui:, CallStatusLbl, ❌ Не чую канал
        GuiControl, CallGui:, CallCrmLbl, Увімкни звук і натисни ще раз
    }
return
