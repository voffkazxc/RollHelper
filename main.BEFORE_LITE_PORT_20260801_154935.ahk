...
RhNoChange:
    ...
    OrderComment := RegExReplace(OrderComment, "i)\s*Решт[уа]\s*з\s*\d+\s*", " ")
    OrderComment := RegExReplace(OrderComment, "\s*\|\s*\|+", " | ")
    if (!InStr(OrderComment, "Без сдач") && !InStr(OrderComment, "Без здач"))
        OrderComment := "Без сдачі | " . OrderComment
    OrderComment := Trim(RegExReplace(OrderComment, "\s{2,}", " "), " |")
    GuiControl, Roll:, OrderComment, %OrderComment%
    GoSub, RhRefreshEnterPreview
return
...
ApplyRollclub:
    ...
    if ((DoAutoCash || DoAutoCard || paymentMethod != "") && !noPayChange) {
        ...
        if (cashX > 0 || cashRelX > 0) {
            ...
            Sleep, % Max(150, SpDly(400))
            IikoClickRelV(cashRelX, cashRelY, cashX, cashY, "Оплата: тип оплати")
            Sleep, % Max(350, SpDly(900))
        }
        ...
        if (_paySearch != "") {
            ...
            Send, %_paySearch%
            Sleep, % Max(250, SpDly(700))
            Send, {Enter}
            Sleep, % Max(300, SpDly(800))
        }
