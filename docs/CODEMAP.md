...
RollClub запускается через `engine_rollclub.ahk`. Базовый пакет содержит тильду, Enter, F1 и `Ctrl+Enter`; F4-дежурство и зоны доставки остаются отдельными дополнениями.

F4 выполняется внутри основного процесса `engine_rollclub.ahk`, а пакет `rollclub-duty` только разрешает эту функцию. Быстрое чтение списка зафиксировано отдельно в `brands/rollclub/server/rollclub_kc_legacy.py`; `packaging/build-rollclub-mvp.ps1` подключает его только к серверу RollClub. Отдельный AHK-процесс дополнения не должен управлять Syrve параллельно с основным пультом.
...
При первом запуске RollClub карта WinAPI переносится из установленной версии, где найдено больше всего сохранённых ролей. В новые ZIP секции `[UiaMap]` и `[UiaHidden]` не упаковываются, поэтому обновление бренда не должно затирать настройку оператора.
