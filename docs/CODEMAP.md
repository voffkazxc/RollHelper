...
> Актуально на 2026-08-18. Этот файл описывает то, что реально существует в коде сейчас.
...
2. `docs/STABLE_BEHAVIOR_CONTRACTS.md` — рабочие сценарии, которые нельзя ломать, известные ловушки и обязательные доказательства.
3. `docs/RELEASE_GUARDRAILS.md` — обязательные границы при выпуске пакетов: что нельзя менять в чужих брендах и модулях.
4. `docs/VERIFICATION_STANDARD.md` — обязательный стандарт доказательства: когда разрешено говорить «работает».
5. `docs/VERIFICATION_REGISTRY.md` — честный статус проверок существующих функций.
6. `STATUS.md` — текущие ограничения, известные проблемы и ближайшая ручная проверка.
5. Документ конкретной подсистемы, если задача относится к зонам, лаунчеру или обзвону.
6. Только после этого — `PLAN.md`, `PROJECT_BACKLOG.md` и `docs/ARCHITECTURE_DECOMPOSITION.md`: в них есть история и целевая архитектура, но не всё уже реализовано.
...
RollClub запускается через `engine_rollclub.ahk`. Базовый пакет содержит тильду, Enter, F1 и `Ctrl+Enter`; дежурство `Ctrl+F4`, зоны доставки и акция первого заказа остаются отдельными дополнениями.

`rollclub-call-auto-accept` разрешает одноразовый автоприём входящего звонка по `Ctrl+F2`. Сам обработчик находится в основном процессе `engine_rollclub.ahk`, а UIA/WinAPI-поиск кнопки — в `lib/IikoUI.ahk`. Дополнение является feature gate, поэтому изменение обработчика требует новой версии пакета RollClub. Активный режим обязательно виден в пульте, отмечен галочкой в трее и фиксируется в `logs/rollclub_call_auto_accept.log`.

`rollclub-first-order-gunkan` включает прежнюю акцию первого заказа из приложения. Маркер `!!!ПЕРШЕМОБ` активирует карточку Гункана и автоматическое пробитие настраиваемого PLU. Без установленного и включённого дополнения базовый RollClub игнорирует маркер и ничего не пробивает.
...
Ctrl+F5 запускает отдельный пакет `rollclub-refund`. Он получает основные данные открытого заказа через `/api/iiko/read`, а `memoEditDeliveryHistory` и `labelOrderSum` читает точечно через UIA. Модуль только формирует и копирует текст возврата, ничего не отправляет автоматически.
...
Текущий опубликованный каталог: `0.1.67`.
Текущий опубликованный релиз лаунчера: `0.1.65`.
Текущий пакет RollHouse в манифесте: `0.1.18`.
Текущее дополнение отчёта в манифесте: `rollhouse-report-load` версии `0.1.17`.
Текущий пакет RollClub: `0.1.29`; дополнение дежурства — `0.1.2`, зоны — `0.1.1`, возврат — `0.1.7`, Гункан — `0.1.0`, автоприём звонка — `0.1.2`.
...
5. После изменения RollClub запустить `python -m unittest discover -s tests -p 'test_rollclub_*.py'`, затем проверить только затронутый реальный сценарий.
6. Не включать отключённые MVP-модули только ради теста базового заказа.

## Документы по подсистемам

- `STATUS.md` — актуальный рабочий статус.
- `docs/STABLE_BEHAVIOR_CONTRACTS.md` — обязательные стабильные сценарии, ошибки прошлого и критерии готовности.
