...
При первом запуске RollClub 0.1.11 карта WinAPI переносится из установленной версии, где найдено больше всего сохранённых ролей. В новые ZIP секции `[UiaMap]` и `[UiaHidden]` не упаковываются, поэтому обновление бренда не должно затирать настройку оператора.
...
Текущий опубликованный релиз лаунчера: `0.1.36`.
Текущий пакет RollHouse в манифесте: `0.1.17`.
Текущее дополнение отчёта в манифесте: `rollhouse-report-load` версии `0.1.17`.
Текущий пакет RollClub после этого прохода: `0.1.11`; его дополнения F4 и зон обновляются отдельно.
