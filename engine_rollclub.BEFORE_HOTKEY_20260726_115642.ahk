...
RcVerifyPoint:
    ...
    ; --- SERVER DIAGNOSTIC PARALLEL RUN ---
    _diagRes := RhGet("/api/iiko/diagnose_point", 20000)
    FileAppend, % "[" A_Now "] DIAGNOSE_POINT_SERVER: " _diagRes "`n", %A_ScriptDir%\siv_debug.log
    diagServerPoint := ""
    if (RegExMatch(_diagRes, """point""\s*:\s*""([^""]+)""", _matchPt)) {
        diagServerPoint := Trim(_matchPt1)
    }
    ; --------------------------------------
    ...
    ; --- SERVER DIAGNOSTIC COMPARE ---
    FileAppend, % "[" A_Now "] POINT_COMPARE old=""" iikoPoint """ server=""" diagServerPoint """`n", %A_ScriptDir%\siv_debug.log
    ; ---------------------------------
