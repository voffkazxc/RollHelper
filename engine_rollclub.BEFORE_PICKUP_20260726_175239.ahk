...
KcDutyToggle:
    ; F4: дежурство — сам ловить перший вільний годний заказ, бере і зупиняється з гучним сигналом
    ...
    {
        FileAppend, % "[" . A_Now . "] F4 IGNORED — під час пробиття (спрацював не фізичний F4?)`n", %A_ScriptDir%\parse_debug.log
        return
    }
    dutyOn := !dutyOn
    if (dutyOn)
...
F4::GoSub, KcDutyToggle    ; F4 — дежурство (цикл): сам лови вільний заказ (повторний F4 — стоп)
...
; ========================================================
CalcPickup:
    CalcType := RcKitchenMinutes("pickup", 40)
    GoSub, ProcessTimeCalc
return
CalcDelivery:
    CalcType := RcKitchenMinutes(RcDeliveryKindFromZone(RcLastZone), 90)
    GoSub, ProcessTimeCalc
return
ProcessTimeCalc:
        ...
        if (autoBurger) {
            ...
            if (RcZonesOk && RcZones.MaxIndex() > 0) {
                if (RcCurrentZoneMapped && RcCurrentKitchen) {
                    matched := RcIikoPointMatchesKitchen(iikoPointClean, RcCurrentKitchen)
...
RcCheckZone:
    ...
    if (RcZonesOk && RcZones.MaxIndex() > 0) {
        ...
        if (zone != "") {
            ...
            _deliveryMin := RcKitchenMinutes(RcDeliveryKindFromZone(zone), 90)
...
RcVerifyPoint:
    ...
    if (!CHECK_POINT_ENABLED || hasPickup || naitiX = 0 || tochkaX = 0) {
        ToolTip, % "Звірка точки ПРОПУЩЕНА -> chk=" . CHECK_POINT_ENABLED . " pickup=" . hasPickup . " naitiX=" . naitiX . " tochkaX=" . tochkaX
        SetTimer, RemoveToolTip, -6000
        return
    }
    ToolTip, Звірка точки: серверна підготовка...
    SoundBeep, 700, 90
    ...
    if (!isOk || sessionId = "") {
        FileAppend, % "[" A_Now "] RcVerifyPoint PREPARE FAIL: " _prepRes "`n", %A_ScriptDir%\siv_debug.log
    }
    
    ToolTip, Звірка точки: тисну "Найти точку"...
    ...
    if (isOk && sessionId != "") {
        ...
        FileAppend, % "[" A_Now "] RcVerifyPoint COLLECT: " _diagRes "`n", %A_ScriptDir%\siv_debug.log
        
        cOk := false
        if (RegExMatch(_diagRes, """ok""\s*:\s*(true|1)", _cOk))
            cOk := true
        ...
        ; Якщо COLLECT успішний і точка прочитана (і вона не порожня), вважаємо успіхом
        if (cOk && pointStr != "") {
            iikoPoint := pointStr
            diagSuccess := true
        } else {
            FileAppend, % "[" A_Now "] RcVerifyPoint COLLECT FAIL (or empty): " _diagRes "`n", %A_ScriptDir%\siv_debug.log
        }
    }
    
    if (!diagSuccess) {
        Sleep, 1000
        ; Закриваємо будь-який попап що може зʼявитись після "Найти точку"
        Loop, 4 {
            if WinExist("Сумма заказа") {
                WinActivate, Сумма заказа
                Sleep, 150
        ...
        Sleep, 600
        ; читаємо поле "Точка*": клік → End → Shift+Home (виділити весь текст без ^a) → ^c
        Clipboard := ""
    ...
    matched := 0
    if (iikoPoint != "") {
        iikoPointClean := Trim(RegExReplace(iikoPoint, "\(.*", ""))
        ; Для самовивозу — порівнюємо по PickupConcept ("Рівне" є в "РК Рівне: Ровно")
        if (hasPickup && pickupPoint != "") {
            _pkc := PickupConcept(pickupPoint)
            if (_pkc != "" && InStr(iikoPointClean, _pkc))
                matched := 1
            if (_pkc != "" && InStr(_pkc, iikoPointClean))
                matched := 1
        }
        ; Для доставки — порівнюємо по KML-зоні
        if (!matched && RcLastZone != "") {
            zoneClean := Trim(RegExReplace(RcLastZone, "[\[\]]", ""))
            if (InStr(iikoPoint, zoneClean) || InStr(zoneClean, iikoPointClean)
                || InStr(iikoPointClean, zoneClean) || InStr(RcLastZone, iikoPointClean))
                matched := 1
        }

        ; Метод 2: по кухням (kTerm)
        if (!matched) {
            dbgkterms := ""
            for _, k in Kitchens {
                kTerm := (k.KmlKey != "") ? k.KmlKey : k.Name
                hasOur := InStr(zoneClean, kTerm)
                hasIiko := InStr(iikoPointClean, kTerm)
                dbgkterms .= "[" . kTerm . ":" . hasOur . "|" . hasIiko . "] "
                if (kTerm != "" && hasOur && hasIiko) {
                    matched := 1
                    break
                }
            }
            FileAppend, % "==== DEBUG kTerms: " . dbgkterms . "`n", %A_ScriptDir%\parse_debug.log
        }

        ; Метод 3: по-словно — будь-яке значуще слово (4+ символи) з зони є в iiko-точці
        if (!matched) {
            Loop, Parse, zoneClean, %A_Space%
            {
                _w := Trim(A_LoopField)
                if (StrLen(_w) >= 4 && InStr(iikoPoint, _w)) {
                    matched := 1
                    break
                }
            }
    ...
    if (matched) {
        ; ✅ Збіг — один короткий високий сигнал + ToolTip "ОК"
        ToolTip, % "✅ Точка ОК: " . iikoPoint
        SoundPlay, %A_ScriptDir%\beep_ok.wav
        SetTimer, RemoveToolTip, -3000
    } else {
        ; ❌ Розбіжність — три сигнали (чітко чутно) + ToolTip з деталями
        _msg := "⚠ ТОЧКА РІЗНИТЬСЯ!" . "`nПульт: " . RcLastZone . "`niiko:  " . iikoPoint
        ToolTip, %_msg%
        SoundPlay, %A_ScriptDir%\beep_err.wav
        Sleep, 550
        SoundPlay, %A_ScriptDir%\beep_err.wav
...
RcLoadZoneMap() {
    ...
    Loop, Parse, SectionNames, `n
    {
        ...
        RcZoneMap[sec] := {Kitchen: k, Type: t}
    }
}

RcIikoPointMatchesKitchen(iikoPoint, kitchen) {
    pt := RegExReplace(iikoPoint, "[^\wа-яА-ЯіІїЇєЄґҐ]", "")
    StringLower, pt, pt
    
    ; check Name
    nm := RegExReplace(kitchen.Name, "[^\wа-яА-ЯіІїЇєЄґҐ]", "")
    StringLower, nm, nm
    if (nm != "" && InStr(pt, nm))
        return 1
        
    ; check KmlKey
    kk := RegExReplace(kitchen.KmlKey, "[^\wа-яА-ЯіІїЇєЄґҐ]", "")
    StringLower, kk, kk
    if (kk != "" && InStr(pt, kk))
        return 1
        
    ; check Address
    ad := RegExReplace(kitchen.Address, "[^\wа-яА-ЯіІїЇєЄґҐ]", "")
    StringLower, ad, ad
    if (ad != "" && InStr(pt, ad))
        return 1
