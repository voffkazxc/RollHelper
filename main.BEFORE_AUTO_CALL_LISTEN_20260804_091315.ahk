...
ShowCallProgress:
    ...
    Gui, CallGui:Add, Button, x10 y+6 w260 h28 gCallListenTest, 🎧 Слух 8с
...
CallListenTest:
    ToolTip, 🎧 Слухаю 8 секунд з каналу Speakers loopback..., 10, 10, 6
    _listenCmd := ComSpec . " /c cd /d """ . A_ScriptDir . """ && python ""tools\call_listener\call_listen.py"" --seconds 8 --device 30 --model small --language auto --beam-size 3"
...
WaitForTalkStart:
    ...
    if (ErrorLevel = 0) {
        ...
        UpdateCallGuiState("talking")
        SetTimer, CallAutoListenAfterGreeting, -6000
...
CallAutoListenAfterGreeting:
    if (!callListMode || callPaused || callFrozen)
        return
    if (callAutoListenBusy)
        return
    callAutoListenBusy := 1
    GuiControl, CallGui:, CallStatusLbl, 🎧 Слухаю клієнта...
    _listenCmd := ComSpec . " /c cd /d """ . A_ScriptDir . """ && python ""tools\call_listener\call_listen.py"" --device 30 --model small --language auto --beam-size 3 --until-silence --seconds 8 --start-timeout 4 --min-speech-seconds 1.5 --silence-seconds 1.0 --rms-threshold 120"
    RunWait, %_listenCmd%, %A_ScriptDir%, Hide
    ...
    if (_decision = "positive") {
        ...
        ToolTip, ✅ Позитив: вмикаю прощання X, 10, 10, 5
        Send, {x}
        Sleep, 3500
        GuiControl, CallGui:, CallStatusLbl, 👂 Чекаю тишу після X...
        _byeCmd := ComSpec . " /c cd /d """ . A_ScriptDir . """ && python ""tools\call_listener\call_listen.py"" --device 30 --until-silence --seconds 3 --start-timeout 1 --min-speech-seconds 0.4 --silence-seconds 1.0 --rms-threshold 120 --no-transcribe"
        ...
        if (_canHangupTest) {
            ...
            ToolTip, ✅ Можна класти трубку (тест без кліка), 10, 10, 5
        } else {
            GuiControl, CallGui:, CallStatusLbl, ⚠️ Після X є звук — ручний режим
            ToolTip, ⚠️ Після X ще є звук/мова — не кладу. Ручний режим, 10, 10, 5
        }
    } else {
        GuiControl, CallGui:, CallStatusLbl, % "👤 Ручний режим: " . _decision
        ToolTip, % "👤 Ручний режим: " . _decision . " / " . _reason . "`n" . SubStr(_text, 1, 120), 10, 10, 5
    }
    callAutoListenBusy := 0
